	...
	public void CreateInputs()
	{
		...
		foreach ( var subgraphInput in subgraphInputs )
		{
			var type = subgraphInput.InputType switch
			{
				...
				InputType.Sampler => typeof( Sampler ),
				InputType.Texture2D => typeof( Texture2DObject ),
				InputType.TextureCube => typeof( TextureCubemapObject ),
				_ => typeof( float )
			};
...
internal class SubgraphNodeControlWidget : ControlWidget
{
	...
	private void Rebuild()
	{
		...
		foreach ( var inputRef in Node.InputReferences )
		{
			...
			{
				// Sampler is not editable in the UI - will use default value from SubgraphInput
				// Skip adding a control widget for this type
			}
			else if ( type == typeof( Texture2DObject ) )
			{
				// Texture2DObject is not editable in the UI - must be connected via node
				// Skip adding a control widget for this type
			}
			else if ( type == typeof( TextureCubemapObject ) )
			{
				// TextureCubemapObject is not editable in the UI - must be connected via node
				// Skip adding a control widget for this type
			}
		}
